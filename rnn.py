...
class BasicRNN:
  ...
  """
    ...
    """
        ...
        dL_db_h, dL_dW_xh, dL_dW_hh = (
            np.zeros(self.b_h.shape),
            np.zeros(self.W_xh.shape),
            np.zeros(self.W_hh.shape),
        )
        ...
        for i in range(n - 1, -1, -1):
            ...
            dL_dW_xh += (
                layer_wise_loss_grads * sigmoid_prime(self.h_t[i + 1]) @ self.x_t[i].T
            )
            dL_dW_hh += (
                layer_wise_loss_grads * sigmoid_prime(self.h_t[i + 1]) @ self.h_t[i].T
            )
            layer_wise_loss_grads = (
                self.W_hh @ layer_wise_loss_grads * sigmoid_prime(self.h_t[i + 1])
            )
